...
class MarkItDownApp(QMainWindow):
    ...
    def _start_file_convert(self):
        ...
        self._start_worker(FileConversionWorker(self.selected_file_path))
    ...
    def _start_text_convert(self):
        ...
        self._start_worker(TextConversionWorker(text, fmt))
    ...
    def _set_converting(self, on):
        ...
        self.btn_clear_result.setEnabled(not on)
    ...
    def _on_finished(self, success, text, error):
        ...
        if self._thread:
            self._thread.quit()
            self._thread.wait(3000)  # 设置超时，防止 UI 无限卡死
            self._thread = None
            self._worker = None
